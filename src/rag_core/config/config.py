...
class Config:
  ...
  EXCLUDED_DIRS: list[str] = os.getenv("EXCLUDED_DIRS", ".git,.DS_Store,.tmp,.venv,.env,.gitignore,.gitmodules,.gitignore").split(",")
  ...
  READ_ENCODINGS: list[str] = [
    item.strip() for item in os.getenv("READ_ENCODINGS", "utf-8,utf-8-sig,utf-16,cp1252").split(",") if item.strip()
  ]
